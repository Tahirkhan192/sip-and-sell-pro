...
-- 1. cash_movements: add payment_source
ALTER TABLE public.cash_movements
  ADD COLUMN IF NOT EXISTS payment_source text NOT NULL DEFAULT 'cash';
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='cash_movements_payment_source_chk') THEN
    ALTER TABLE public.cash_movements ADD CONSTRAINT cash_movements_payment_source_chk
      CHECK (payment_source IN ('cash','online'));
  END IF;
END $$;

-- 2. Rebuild daily_closing_summary to include online in/out
...
-- 3. Stock to Expense transfer RPC
CREATE OR REPLACE FUNCTION public.stock_to_expense_transfer(
  _product_id uuid,
  _stock_item_id uuid,
  _quantity numeric,
  _expense_category text,
  _reason text,
  _notes text,
  _date date
...
BEGIN
  ...
  INSERT INTO public.expenses (date, category, amount, description, payment_method)
  VALUES (
  ...
  RETURN v_expense_id;
END; $$;
GRANT EXECUTE ON FUNCTION public.stock_to_expense_transfer(uuid,uuid,numeric,text,text,text,date) TO authenticated;
