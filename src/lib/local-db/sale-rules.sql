-- Bill-saving rules, kept identical to public/seed/schema.sql.
-- These are re-applied on every start so a device set up before an update
-- still uses the current rules (an existing local database never re-runs the
-- installation script).
