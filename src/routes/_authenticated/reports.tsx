      ...
      <CardContent className="p-0">
        <Table>
          <TableBody>
            ...
            <TableRow className="font-medium"><TableCell>Total Net Profit Before Staff Salary</TableCell><TableCell className={"text-right " + (((data?.netProfit ?? 0) + (data?.staffSalaryCost ?? 0)) >= 0 ? "text-primary" : "text-destructive")}>{money((data?.netProfit ?? 0) + (data?.staffSalaryCost ?? 0))}</TableCell></TableRow>
            <TableRow><TableCell>− Total Staff Salary (present days × monthly ÷ 30)</TableCell><TableCell className="text-right">{money(data?.staffSalaryCost)}</TableCell></TableRow>
